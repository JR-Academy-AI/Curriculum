...
// Why（定义需求的天花板）→ What（产品分析：Pages/CRUD/Components/Flow/数据关系）
// → How（ADLC + PRD 六块模板 + 动手写 Lab）→ Infrastructure（Rules/Repo）→ Workshop。
...
import L2P01h from './components/slides/L2P01h_DataRelations';  // 数据怎么想：先画关系，再定字段
...
import L2P04c from './components/slides/L2P04c_PRDLab';        // Lab：学生自己写 PRD
...
export default function App() {
	return (
		<SlideEngine>
			...
			<L2P01h />{/* 数据怎么想：先画关系，再定字段 */}
			<S14b />{/* Design System = 视觉 SoT */}

			{/* How：ADLC 方法论 + 写 PRD（这时 Pages/CRUD/Flow/数据关系都已经学过） */}
			<S16d />{/* ADLC 闭环全景 */}
			<L2P03 />{/* 整份 PRD 交给一个 agent */}
			<L2P04 />{/* PRD 六块模板 */}
			<L2P04b />{/* PRD 必须具备什么 */}
			<L2P04c />{/* Lab：现在你来写 PRD */}
