...
| 价 | **US$399** · 首期 early-bird **US$299**（限量） | 待定 |
...
- **列表价 US$399 + 首期 early-bird US$299（限量）**（美金，非澳元/人民币）。2026-07-08 Lightman 拍板调价（原 US$220 定价过低）。已写入 `outline.json` → `program.tuition: 399` + `program.promoTuition: 299` + `currency: "USD"`。
- ⚠️ **货币口径注意**：其它课程价格数字是澳元/人民币，本课是**美金**。同步到 production 时必须把该 program 的货币设为 **USD**（`currency` 字段是本地文档标注，sync 未必透传——落地前务必在产品/后端确认币种，否则 399 会被当成 A$399 或 ¥399）。
- 定价逻辑：名额硬成本（Pearson 监考）约 US$125 且报名通道全市场独家；华人市场 PMP/AWS 认证培训普遍 US$280–490，它们还没有"个人报不了名"的独占性；价格过低反而加重"是不是只发我一堆 PDF"的疑虑。early-bird 给「成为全球华人首批 CCA-F 持证人」口号配上行动紧迫感，首期成交案例到手后回列表价。
