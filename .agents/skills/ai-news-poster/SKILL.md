...
**参考实现 + 完整模板**：`curriculum/.Codex/skills/mp-article/SKILL.md` 的「🚨 硬性要求：封面/海报生成器必须用原生 Canvas 2D」章节 + `ai-engineer-bootcamp/public/mp-article/cover.html`（commit dd26b8d）。
...
1. **注册到 `curriculum/posters.html`**（AGENTS.md 强制规则 2）
   - 在顶部"工具/资产集合"section 插入入口卡片指向 `./ai-news-posters/`
2. **更新 `.github/workflows/deploy.yml`**（AGENTS.md 强制规则 1）
   - 在 Assemble 步骤加：
     ```yaml
...
在 `https://Codex.ai/code/scheduled` 串联：
```
