...
//   结构与文案沿用 ai-solo-founder-w4 的同名页，保证系列内一致。
//   ⚠️ 只改了两处：① 高亮周 W4 → W3；② W3 的 chip 文案改为本节真实内容
//      （W4 deck 里 W3 写的是「访谈真实客户」，那其实是 W2 自学 L08 的动作，本节是算账不是访谈）。
interface Week {
	w: string;
	name: string;
	desc: string;
...
const PHASES: { no: string; title: string; range: string; desc: string; bg: string; weeks: Week[] }[] = [
	{
		...
		weeks: [
			...
			{ w: 'W3', name: '这是不是一门好生意', desc: '算清楚它到底赚不赚钱，写下继续、调整还是换' },
...
function WeekChip({ week, index, today, done }: { week: Week; index: number; today: boolean; done: boolean }) {
	return (
		<motion.div
		...
		>
			<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 4 }}>
				...
				{done ? (
					<span style={{ fontFamily: fonts.mono, fontSize: 11, fontWeight: 700, background: '#ddd', color: '#666', padding: '1px 6px' }}>
						已上
					</span>
				) : null}
			</div>
		...
		<Slide bg={colors.white}>
			<Body style={{ padding: '36px 60px 30px' }}>
				<SlideHead
					...
					sub="本期 W3 与 W4 调了个个儿——上周先把东西做出来了，今天回头算它值不值得做下去。"
				/>
						...
						>
							...
							<div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 10 }}>
								{p.weeks.map((wk) => {
									const i = running;
									running += 1;
									return <WeekChip key={wk.w} week={wk} index={i} today={wk.w === 'W3'} done={DONE.includes(wk.w)} />;
								})}
							</div>
						</div>
					))}
