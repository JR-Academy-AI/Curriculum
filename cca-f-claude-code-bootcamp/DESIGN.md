...
| Phase | 主题 | 关键实战产出 |
|---|---|---|
| P0 认证+报名+装机 | 搞懂考什么 + 激活名额 + 装 Claude Code | 手里有名额 + 本机能跑 Claude Code |
...
| P4 Domain 3 Claude Code Config | hooks/命令/settings（权重最高） | 配好 hooks + 2 个 slash command |
...
- **"你只差一张证、已经会用 Claude Code" → 考试直通包（US$220）**
...
价格**待定**。`outline.json` 里现填的 `tuition: 1699 / promoTuition: 1299` 是占位建议价，**未定稿**。定价锚点：直通包 US$220（含名额），本课在其之上加 8 场直播 + 40h 真实环境实战 + 答疑。定稿后回填 `program.tuition`，并与直通包一致用 **USD** 币种。
