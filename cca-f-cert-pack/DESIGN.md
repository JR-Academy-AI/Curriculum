...
- **站内认证详情页价格卡：定价锚点 US$399（直通包价）+「立即购买」**（Lightman 2026-07-12 决定，美元锚定）。
  - 落点：`certification.pricing.singlePrice = 614`，三处同步（前端 `data/ccar-f/certification.ts`、后端 `seed-data/ccar-f.seed.ts`、prod 库 import-static）。
  - 🚨🚨 **存的是 614 不是 399**：系统基准货币 AUD、固定汇率 1 AUD = US$0.65（`exchangeRate.ts`）。要美元用户精确看到 US$399，必须反推存 AUD 基准值 399/0.65 ≈ 614。各币种展示：🇺🇸 **US$399** / 🇦🇺 A$614 / 🇨🇳 ¥2886 / 🇬🇧 £319。改价 = 改「目标美元数 ÷ 0.65」，别直接填美元数字（会被再乘 0.65 打折成 US$259）。
  - 系统当前 C 端 cert 无 per-currency 独立定价能力（详情页价格卡只做 AUD 基准 × 固定汇率的机械换算），所以非美元币种数字不圆整是系统限制，不是配置错误。
  - ⚠️ 这是**直通包产品价**（路线 + 原创冲刺件 + 陪跑 + 开通 Partner Academy 账号），**不是官方考试费**（$125 USD 学员自付，站内外都不写这个数字）。
