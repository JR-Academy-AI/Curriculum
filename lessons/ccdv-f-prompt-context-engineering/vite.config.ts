...
function manifestPlugin(base: string): Plugin { return { name: "jr-classroom-manifest", generateBundle(_options, bundle) {
  ...
  this.emitFile({ type: "asset", fileName: "manifest.json", source: JSON.stringify({ schemaVersion: 1, bridgeVersion: classroomConfig.bridgeVersion, deckId: classroomConfig.deckId, releaseId, sourceCommit, checksum: checksum.digest("hex"), title: classroomConfig.title, status: classroomConfig.status, entryPath: `${base}index.html`, entryUrl, slideCount: classroomConfig.slides.length, slides: classroomConfig.slides.map(({ thumbnailPath, ...slide }, index) => ({ ...slide, thumbnailUrl: origin ? `${origin}${base}${thumbnailPath}` : `${base}${thumbnailPath}`, actions: slide.actions.map(({ audioKey, ...action }) => ({ ...action, audioUrl: audioBase ? `${audioBase}/${audioKey}` : `${base}audio/${audioKey}` })), index })) }, null, 2) });
} }; }
const configuredBase = process.env.CLASSROOM_RELEASE_BASE || (process.env.NODE_ENV === "production" ? "/curriculum/lessons/ccdv-f-prompt-context-engineering/" : "/");
