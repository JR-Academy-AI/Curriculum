...
export function Inner({ children, center, split, style }: { children: ReactNode; center?: boolean; split?: boolean; style?: CSSProperties }) {
	return (
		<div style={{
			// minHeight 而不是固定 height：内容偏高时往下长（Slide 居中，所以是上下对称长），
			// 而不是被 Slide 的 overflow:hidden 静默裁掉底部。maxHeight 兜住不出画布。
			width: '90%', maxWidth: 1400, minHeight: '85%', maxHeight: '94%', display: 'flex', gap: 48, padding: 40,
