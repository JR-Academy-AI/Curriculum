...
function Closing({ s }: { s: DeckSlideContent }) {
	return (
		<Slide bg={colors.red}>
			<div style={{ width: '100%', height: '100%', padding: '82px 104px 72px', position: 'relative', display: 'flex', flexDirection: 'column' }}>
				<div style={{ position: 'absolute', right: -120, top: -150, width: 500, height: 500, background: colors.yellow, borderRadius: '50%', border: '5px solid #000' }} />
				...
				<motion.h2 {...fadeUp(.12)} style={{
					...
					真正要建立的，是你的
					<span style={{ display: 'block', background: colors.yellow, padding: '0 20px 8px', width: 'fit-content', marginTop: 10 }}>可被需要性</span>
				</motion.h2>
				<motion.p {...fadeUp(.28)} style={{ fontSize: 27, fontWeight: 800, marginTop: 28 }}>{s.note}</motion.p>
					...
					{(s.links ?? []).map((link, i) => (
						...
						>
							<div>
								...
								<div style={{ fontFamily: fonts.heading, fontSize: 34, fontWeight: 900, marginTop: 7 }}>{link.meta}</div>
							</div>
