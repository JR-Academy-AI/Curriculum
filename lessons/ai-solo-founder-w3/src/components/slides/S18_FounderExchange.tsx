...
// 中段 30min Founder Exchange —— 每节现场课的固定环节
// 来源：HANDOVER_DECKS.md §4.1（Lightman 2026-08-01 定案：位置在中段，不是开场）
// 三段节奏沿用 ai-solo-founder-w4 S16，本周把主题收到「算账」上
const FLOW = [
...
export default function S18_FounderExchange() {
	return (
		<Slide bg={colors.yellow}>
			<Body>
				...
				<div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 18, marginBottom: 18 }}>
					{FLOW.map((f) => (
						<div key={f.t} style={{ background: colors.white, border, boxShadow: shadow, padding: '18px 20px', minHeight: 170 }}>
				...
				<div style={{ background: colors.dark, border, padding: '16px 22px' }}>
					<div style={{ fontFamily: fonts.mono, fontSize: 13, color: colors.yellow, letterSpacing: 1.5, marginBottom: 10 }}>
						本周特别版：拿着你刚打完分的那张表去聊
					</div>
					<div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 18, color: colors.white, fontSize: 16.5, lineHeight: 1.5 }}>
						<div>
							<b style={{ color: colors.yellow }}>1 ·</b> 把你最低分那把尺子念出来，问对方「你会怎么补」
						</div>
						<div>
							<b style={{ color: colors.yellow }}>2 ·</b> 找一个跟你客户群重叠的人——他可能就是你的第一个引荐来源
						</div>
						<div>
							<b style={{ color: colors.yellow }}>3 ·</b> 报名下半场上台：现在举手，还剩几个位置
						</div>
					</div>
				</div>
