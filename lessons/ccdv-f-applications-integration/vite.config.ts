  ...
  return {
    ...
    generateBundle(_options, bundle) {
      ...
      for (const fileName of Object.keys(bundle).sort()) {
        ...
        checksum.update(output.type === "asset" ? (typeof output.source === "string" ? output.source : Buffer.from(output.source)) : output.code);
      }
      this.emitFile({
        type: "asset",
        ...
        source: JSON.stringify({
          schemaVersion: 1,
          bridgeVersion: classroomConfig.bridgeVersion,
          deckId: classroomConfig.deckId,
          releaseId,
          sourceCommit,
          checksum: checksum.digest("hex"),
          title: classroomConfig.title,
          status: classroomConfig.status,
...
const configuredBase = process.env.CLASSROOM_RELEASE_BASE || (process.env.NODE_ENV === "production" ? "/curriculum/lessons/ccdv-f-applications-integration/" : "/");
