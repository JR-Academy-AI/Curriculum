...
function runtimeFromUrl(): Runtime | null {
  ...
  if (!deckId || !releaseId || !parentOrigin)
    throw new Error("Classroom identity is incomplete");
  if (deckId !== classroomConfig.deckId)
    throw new Error(`Deck identity mismatch: ${deckId}`);
  if (new URL(parentOrigin).origin !== parentOrigin)
    throw new Error("parentOrigin must be an origin");
  return { deckId, releaseId, parentOrigin };
}

export function useClassroomBridge(
  onLoadSlide: (index: number) => void,
  slideCount: number
) {
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (data.type === "JR_CLASSROOM_LOAD" && Number.isInteger(data.slideIndex)) {
        ...
        if (index < 0 || index >= slideCount) {
          post({
            ...
            message: `Slide ${index} is outside this release`
          });
          return;
        }
        onLoadSlide(index);
      ...
      } else if (data.type === "JR_CLASSROOM_THEME") {
        window.dispatchEvent(new CustomEvent("jr-classroom-theme", { detail: data }));
