...
const manifestResponse = await fetch(`${deckUrl}/manifest.json`);
const manifestText = await manifestResponse.text();
let manifest;
try {
  manifest = JSON.parse(manifestText);
} catch {
  const fixture = await (await fetch(`${deckUrl}/local-classroom.json`)).json();
  manifest = {
    deckId: fixture.presentation.deckId,
    releaseId: fixture.presentation.releaseId,
    slides: fixture.steps.map((step, index) => ({ id: step.id, index }))
  };
}
const parentPort = Number(process.env.CLASSROOM_QA_PARENT_PORT || 4175);
  ...
  for (const viewport of viewports) {
    ...
    for (const slide of manifest.slides) {
      ...
      const layout = await frame.evaluate(() => {
        ...
        const landmarks = [".studio-header", ".lesson-board", ".chapter-heading h1", ".takeaway"].map(selector => {
